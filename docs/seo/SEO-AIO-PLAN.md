...
| F13 | **Medium** | HTML ships **uncompressed** (~40 KB/page) because of the deliberate `no-transform` workaround for a Hostinger CDN bug. Documented and justified, but it is a real TTFB/LCP cost that needs a scheduled re-test. |
