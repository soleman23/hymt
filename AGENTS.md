...
This project is indexed by GitNexus as **hymt** (7032 symbols, 22528 relationships, 44 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- Every tool that writes `images-b64/MANIFEST.json` must use
  `tools/image-manifest.mjs`. It sorts by target and writes indent-1 LF JSON;
  run it directly to normalize the manifest after resolving an entry conflict.
