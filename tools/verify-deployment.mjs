   ...
   asserting the two match is what proves astro copied it at all. */
{
  ...
  if (distHt === null) {
    ...
    if (!failures.some((f) => f.check === "htaccess-headers")) {
      ...
      notes.push(`.htaccess shipped with ${HTACCESS_SECURITY_HEADERS.length} security headers + host-scoped HSTS (#79), the staging noindex, the /sitemap.xml alias (#192), ${CSP_DIRECTIVES.length} CSP directives, UTF-8 default charset, and immutable caching limited to content-hashed CSS/JS (#193)`);
    }
  }
}
